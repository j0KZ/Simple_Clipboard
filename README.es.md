...
- Ignora 1Password, Bitwarden, KeePass, Keeper, Contraseñas de Apple y todo lo marcado
  como `org.nspasteboard.ConcealedType` / `TransientType` / `AutoGeneratedType`.

Las apps de dictado son una excepción a propósito. Wispr Flow marca lo que dicta como
`ConcealedType` —el mismo distintivo que usan los gestores de contraseñas—, pero además
restaura tu portapapeles anterior medio segundo después. Esa restauración es la pista: un
recorte privado sólo entra al historial si el portapapeles vuelve *exactamente* a lo que
tenía antes, dentro de 1,5 s. Un gestor de contraseñas nunca hace eso, así que lo suyo
queda fuera.
...
Sólo corre una instancia a la vez: la segunda detecta a la primera y se retira. Ambas
registrarían `⌥⌘V` sin dar error pero sólo una lo recibiría, y el síntoma sería que el
atajo no hace nada.
...
No pide nada más: ni cámara, ni micrófono, ni ubicación, ni contactos, ni grabación de
pantalla, ni Apple Events.
